  ...
  class Hooks < Redmine::Hook::ViewListener
    ...
    def save_sub_task_list(items, issue)
      ...
      items.each do |item|
        ...
        sub_task.due_date = item["due_date"].presence || issue.due_date
        ...
        sub_task.start_date = item["start_date"].presence || issue.start_date
    ...
    def items_params(context={})
      if context[:params][:items].present?
        context[:params].require(:items).map do |item|
          item.permit(:text, :estimated_hours, :start_date, :due_date) if !item[:text].blank?
        end.compact
      else
        {}
      end
